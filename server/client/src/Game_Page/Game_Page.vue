	...
	methods: {
      ...
      this.cardHover(this[deck][number]);
		...
		moveCard({zone, oldZone, under}) {
			...
			/* Здесь пишем логику перемещения карты с одной зоны в другую на фронте. После перемещения, отправить 
				событие на бэк, чтобы синхронизировать изменения. Возможно, логику с бэка стоит просто переместить сюда,
				а на бэк отправлять только изменившиеся зоны - и обрабатывать.
			*/
		},
	},
	created() {
